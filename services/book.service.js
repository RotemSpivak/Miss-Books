...
function _loadFromStorage(books) {
...
function addGoogleBook(gBook) {
  const book = {
    ...
    thumbnail: gBook.volumeInfo.imageLinks?  gBook.volumeInfo.imageLinks.thumbnail : `https://www.google.com/url?sa=i&url=https%3A%2F%2Fedit.org%2Fbook-covers&psig=AOvVaw0GslOqNUtw7i6mtG7PVd6p&ust=1651085154899000&source=images&cd=vfe&ved=0CAwQjRxqFwoTCICL-9OxsvcCFQAAAAAdAAAAABAD`,
    language: gBook.volumeInfo.language,
    listPrice: {
      amount: utilService.getRandomIntInclusive(5,200),
      currencyCode: Math.random()<0.333? 'EUR' : Math.random()>0.666? "USD" : "ILS",
      isOnSale: Math.random()<0.5 
      
    }
  }
  const savedBooks = storageService.loadFromStorage(STORAGE_KEY)
  savedBooks.push(book)
  storageService.saveToStorage(STORAGE_KEY, savedBooks)
...
function getNextBookId(bookId) {
  ...
  console.log('next bookIdx', nextBookIdx)
